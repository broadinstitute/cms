...
// for a putative selpop and an arbitrary number of comparison populations, pulls and collates all component score statistics for a region. (assuming uniform prior)
// -->COMPILE: gcc -o combine_cms_scores_multiplepops_region -O0 -ggdb3 -lm -Wall combine_cms_scores_multiplepops_region.c cms_data.c
...
int main(int argc, char **argv) {
	...
	FILE *outf=NULL;
	int nComparisons, iComp, isnp, iArg;
	char outfilename[256]; 
	double thisihs, thisihh; // per-pop
	double thisfst, thisxpehh, thisdelDaf;
	double compLikeRatio, numerator, denominator;
	char* simSelPop;
	char* simAltPop; //for indexing likes tables based on 1KG pops
	char popCompString[3];
	float prior; // = 1/nsnp in region; uniform

	if (argc < 6) {
		fprintf(stderr, "Usage: ./combine_cms_scores_multiplepops_region <bp start> <bp end> <chrom> <selpop> <poplikes> <otherpop1> <...> \n");
		exit(0);
	}
	...
	strcpy(outfilename, "/idi/sabeti-scratch/jvitti/synth/cms_combined/regions/chr");
	...
	strcat(outfilename, argv[5]);
	strcat(outfilename, ".cms.out");
	...
	//popCompString = getPopCompString()
	simSelPop = "1";
	simAltPop = "2"; // hardCODED FOR NOW
	sprintf(popCompString, "1_2");

	sprintf(delihh_hit_filename, likeloc);
	strcat(delihh_hit_filename, "delihh_default_112115_825am_");
	strcat(delihh_hit_filename, simSelPop);
	...
	strcat(delihh_miss_filename, simSelPop);
	...
	strcat(ihs_hit_filename, simSelPop);
	...
	strcat(ihs_miss_filename, simSelPop);
	...
	strcat(xpehh_hit_filename, popCompString);
	...
	strcat(xpehh_miss_filename, popCompString);
	...
	strcat(deldaf_hit_filename, popCompString);
	...
	strcat(deldaf_miss_filename, popCompString);
	...
	strcat(fst_hit_filename, popCompString);
	...
	strcat(fst_miss_filename, popCompString);
	strcat(fst_miss_filename, "_likes_neut.txt");
