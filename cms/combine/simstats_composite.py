...
runPrefix = "composite_exclude_110716"
basedir = "/idi/sabeti-scratch/jvitti/scores_composite2/"
pairbasedir = "/idi/sabeti-scratch/jvitti/scores_composite/"
likesdir = '/idi/sabeti-scratch/jvitti/likes_110516/'
...
def main():
	for model in models:
		...
		for selpop in pops:
			...
			for sel_freq_bin in sel_freq_bins:
				...
				for irep in range(1, numPerBin +1):
					...
					if len(inscorefilelist) > 0:
						...
						outfile = basedir + model + "/sel" + str(selpop) + "/sel_" + str(sel_freq_bin) + "/rep" + str(irep) + "_" + str(selpop) +"_vsneut.cms.out.exclude"
						#if True:
			...
			for irep in range(1, numPerBin +1):	
				...
				if len(neutinscorefilelist) > 0:
					...
					for filename in neutinscorefilelist:
						scorefilelist += filename + ","
					scorefilelist = scorefilelist[:-1]
					outfile = basedir  + model + "/neut/rep" + str(irep) + "_" + str(selpop) +".cms.out.exclude"
			...
			writeJobsToTaskArray_fromArglist(cmd, arguments, runPrefix + "_" + str(model) + "_sel" + str(selpop), discardOut=False)
