 // adapt from combine_gw_extended 
// 01.15.19: exclude maf-zero sites (which enter via outgroup freqfile; i.e. they are variant in outgroups)
//gcc -c collate_scores_v2_maf.c
//gcc -O0 -ggdb3 -lm -lz -Wall -o collate_scores_v2_maf collate_scores_v2_maf.o cms_data_vers.c
...
int main(int argc, char **argv) {
	...
	const float prior = .1; // this is an arbitrary value (test)
	...
	double thisfst, thisxpehh, thisdelDaf, thisdaf, thismaf;
	...
	if (argc <= 3) {
		fprintf(stderr, "Usage: ./combine_gw_extended <savefilename> <input_pair_file1> ...\n");
		exit(0);
	}
	nComparisons = argc - 2;
	...
	*/
	...
	//	assert(outf2 != NULL);
	//	fprintf(outf2, "physPos\tgenPos\tpopDAF\tlike_iHS\tlike_nsl\tlike_deliHH\tlike_H12\tlike_iSAFE\tlike_xp-ehh\tlikefst\tlikedelDAF\tcompLikeRatio_CMS\tcompLike_prior.1\n");	
	//} //end if write likes
	for (isnp = 0; isnp < score_data.nsnps; isnp++){
		...
		//fprintf(stderr)
		//PRINT ALL COMPARISON VALUES 
		thisxpehh = compareXp(&score_data, isnp);
		//fprintf(stderr, "xp: %f\n", thisxpehh); //debug

		//DEBUG
		if (score_data.physpos[iComp][isnp] == 191018477){
			fprintf(stderr,"%f\n", score_data.xp_normed[0][isnp]);
			fprintf(stderr,"%f\n", score_data.xp_normed[1][isnp]);
			fprintf(stderr,"%f\n", score_data.xp_normed[2][isnp]);
			fprintf(stderr,"%f\n", thisxpehh);
		} // END DEBUG

		//thisfst = compareFst_PBS(&score_data, isnp);
		//thisdelDaf = comparedelDaf_outgroup_ave(&score_data, isnp);
		thisfst = compareFst(&score_data, isnp);
		thisdelDaf = comparedelDaf(&score_data, isnp);
		...
		if (thisdaf == 0){proceed=1;}  // variant in outgroups
		compLikeRatio = 0;
		//if still a go...
		if(proceed == 0){
			compLikeRatio = 1;
			...
			/////////////////////
			/// GET CMS SCORE ///	-- as posterior estimate
			/////////////////////		
			...
			*/
			if (thisdaf < .5){thismaf = thisdaf;}
			else{thismaf = 1.0-thisdaf;}

			fprintf(outf, "%d\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t\n", score_data.physpos[iComp][isnp], thismaf, thisihs, thisihh, thisnsl, thisH12, thisH2H1, thisiSAFE, thisxpehh, thisfst, thisdelDaf);//, compLikeRatio, compLike);
			
			/*if (score_data.physpos[iComp][isnp] == 191018477){
				fprintf(stderr, "%d\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t\n", score_data.physpos[iComp][isnp], thisdaf, thisihs, thisihh, thisnsl, thisH12, thisH2H1, thisiSAFE, thisxpehh, thisfst, thisdelDaf);//, compLikeRatio, compLike);
				fprintf(stderr,"%f\n", score_data.xp_normed[0][isnp]);
				fprintf(stderr,"%f\n", score_data.xp_normed[1][isnp]);
				fprintf(stderr,"%f\n", score_data.xp_normed[2][isnp]);
				fprintf(stderr,"%f\n", thisxpehh);
			...
			*/
			...
			//fprintf(outf, "%d\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t\n", score_data.physpos[iComp][isnp], score_data.genpos[iComp][isnp], thisdaf, thisihs, thisnsl,  thisihh, thisH12, thisiSAFE, thisxpehh, thisfst, thisdelDaf);//, compLikeRatio, compLike);
			//if (writeLikes == 0){fprintf(outf2, "%d\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%e\t%f\n", score_data.physpos[iComp][isnp], score_data.genpos[iComp][isnp], thisdaf, ihs_bf, nsl_bf, delihh_bf, H12_bf, iSAFE_bf, xpehh_bf, fst_bf, deldaf_bf, compLikeRatio, compLike);} //end if write likes
		...
		//else{fprintf(stderr, "passed on snp");}
	} // end isnp
	fclose(outf);
	fprintf(stderr, "\nWrote CMS scores to %s\n", outfilename);
	//if (writeLikes == 0){fclose(outf2);fprintf(stderr, "Wrote score decomposition to %s\n", outfilename_likes);}
