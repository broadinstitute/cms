...
writedir = "/idi/sabeti-scratch/jvitti/remodel/run4/"
...
def main():
	if len(sys.argv) != 5:
		print('call program as: python remodel_freqs_4pop.py {model} {regime} {selpop} {irep}')
		sys.exit(0)
	model = sys.argv[1]
	regime = sys.argv[2]
	...
	replicate_idstring = "rep" + str(irep) #+"_pop" + str(thispop)
	...
	for altpop in altpops:
		...
		fstdeldaf_outfilename = basedir + "freqs/"  + replicate_idstring + "_pop" + str(thispop) + "_vs" + str(altpop)
