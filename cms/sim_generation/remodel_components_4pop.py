...
writedir = "/idi/sabeti-scratch/jvitti/remodel/run2/"
...
def mustCreate(filename):
	mustCreate = True
	gz_filename = filename + ".gz"
	if os.path.isfile(filename) and os.path.getsize(filename) > 100:
		mustCreate = False
	elif os.path.isfile(gz_filename) and os.path.getsize(gz_filename) > 100:
		mustCreate = False
	return mustCreate

def main():
	if len(sys.argv) != 5:
		print('call program as: python remodel_components_4pop.py {model} {regime} {selpop} {irep}')
		sys.exit(0)
	model = sys.argv[1]
	regime = sys.argv[2]
	...
	print(ihs_fullcmd)
	if mustCreate(ihs_unnormedfile):
	#if not os.path.isfile(ihs_unnormedfile) and not os.path.isfile(ihs_unnormedfile + ".gz"):
		execute(ihs_fullcmd)
	
	delihh_commandstring = "python3 " + cmsdir + "composite.py delihh_from_ihs"
	...
	if mustCreate(delihh_unnormedfile):
		execute(delihh_fullcmd)		
	
	nsl_commandstring = "python3 " + cmsdir + "scans.py selscan_nsl" 
	...
	if mustCreate(nsl_unnormedfilename):
		execute(nsl_fullcmd)
	...
	for altpop in altpops:
		xpehh_commandstring = "python3 " + cmsdir + "scans.py selscan_xpehh --threads 7"
		...
		print(xpehh_fullcmd)
		if mustCreate(xpehh_unnormedfile):
		...
		if mustCreate(fstdeldaf_outfilename):
