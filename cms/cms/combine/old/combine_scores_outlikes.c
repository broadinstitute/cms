...
// (rather than component score) as column output		vitti@broadinstitute.org
// gcc -O0 -ggdb3 -lm -Wall -o combine_scores_outlikes combine_scores_outlikes.c cms_data.c
// ./combine_scores_outlikes test_out.txt test_masterlikes_params.txt testpair1.txt testpair2.txt
...
int main(int argc, char **argv) {
	...
	if (argc <= 3) {
		fprintf(stderr, "Usage: ./combine_scores_outlikes <savefilename> <cms_run_paramfile> <input_pair_file1> ...\n");
		exit(0);
	}
	nComparisons = argc - 3;
	...
	for (isnp = 0; isnp < score_data.nsnps; isnp++){
		...
		if(proceed == 0){
			...
			fprintf(outf, "%d\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%f\t%e\n", score_data.physpos[iComp][isnp], score_data.genpos[iComp][isnp], thisdaf, ihs_bf, delihh_bf, nsl_bf, xpehh_bf, fst_bf, deldaf_bf, compLikeRatio);
		}//end if-a-go
	} // end isnp
	fclose(outf);
