...
#colorDict = {'LWK':'#CC9933', 'GWD':'#FFB900', 'MSL':'#E1B919', 'YRI':'#FFB933'}
colorDict = {'LWK':'gold', 'GWD':'dodgerblue', 'MSL':'springgreen', 'YRI':'mediumorchid'}
...
def draw_tree(paramDict, fullsaveloc, recent = False, veryrecent = False, printvals = True):
	...
	LWK_axis = 0 + leaf_widths[0]/2 
	GWD_axis = (nbuffer + leaf_widths[0]) + (leaf_widths[1]/2)
	YRI_axis = (2*nbuffer + leaf_widths[0] + leaf_widths[1]) + (leaf_widths[3]/2)
	MSL_axis = (3*nbuffer + leaf_widths[0] + leaf_widths[1] + leaf_widths[3]) + (leaf_widths[2]/2)
	xlims = [0 - nbuffer, MSL_axis + leaf_widths[2]/2 + nbuffer]

	ax2 = plt.subplot2grid((5,10), (0, 0), colspan = 10, rowspan =10)

	LWK_Ne = paramDict[('Ne', 1)] 
	LWK_Ne_times = paramDict[('Ne_times', 1)]
	LWK_Ne_change = paramDict[('Ne_change', 1)]
	GWD_Ne = paramDict[('Ne', 2)]
	GWD_Ne_times = paramDict[('Ne_times', 2)]
	GWD_Ne_change = paramDict[('Ne_change', 2)]	
	MSL_Ne = paramDict[('Ne', 3)]
	MSL_Ne_times = paramDict[('Ne_times', 3)]
	MSL_Ne_change = paramDict[('Ne_change', 3)]	
	YRI_Ne = paramDict[('Ne', 4)] 
	YRI_Ne_times = paramDict[('Ne_times', 4)]
	YRI_Ne_change = paramDict[('Ne_change', 4)]

	LWK_bn = paramDict[('bn', 1)] 
	LWK_bn_times = paramDict[('bn_times', 1)]
	GWD_bn = paramDict[('bn', 2)]
	GWD_bn_times = paramDict[('bn_times', 2)]
	MSL_bn = paramDict[('bn', 3)]
	MSL_bn_times = paramDict[('bn_times', 3)]
	YRI_bn = paramDict[('bn', 4)] 
	YRI_bn_times = paramDict[('bn_times', 4)]
	...
	if paramDict[('Ne_change', 1)][0] != 'exp':
		rect1 = drawRectangle(LWK_axis, 0, leaf_widths[0], LWK_Ne_times[0], colorDict['LWK'], 'LWK', limits, False)
		ax2.add_patch(rect1)
	if paramDict[('Ne_change', 2)][0] != 'exp':	
		rect2 = drawRectangle(GWD_axis, 0, leaf_widths[1], GWD_Ne_times[0], colorDict['GWD'], 'GWD', limits, False)
		ax2.add_patch(rect2)
	if paramDict[('Ne_change', 3)][0] != 'exp':		
		rect3 = drawRectangle(MSL_axis, 0, leaf_widths[2], MSL_Ne_times[0], colorDict['MSL'], 'MSL', limits, False)
		ax2.add_patch(rect3)
	if paramDict[('Ne_change', 4)][0] != 'exp':	
		rect4 = drawRectangle(YRI_axis, 0, leaf_widths[3], YRI_Ne_times[0], colorDict['YRI'], 'YRI', limits, False)
		ax2.add_patch(rect4)
	...
	coalpops = ['LWK', 'GWD', 'MSL', 'YRI']
	travellendict = {'GWD':1, 'MSL':1, 'YRI':1}
	...
	ancrect = drawRectangle(LWK_axis,LWK_Ne_times[-1], LWK_Ne[-1], 100000, colorDict['LWK'], 'anc', limits, printvals)
	ancexprect = drawRectangle(LWK_axis, LWK_Ne_times[-2], LWK_Ne[-2], LWK_Ne_times[-1] - LWK_Ne_times[-2], colorDict['LWK'], '', limits, printvals)
	YRI_split = paramDict[('split', 4)][0]
	GWD_split = paramDict[('split', 2)][0]
	if veryrecent == True:
		ylims = [0,YRI_split+100]
	elif recent == True:
		ylims = [0,GWD_split+500]
	else:
	...
	plt.xticks([LWK_axis, GWD_axis, YRI_axis, MSL_axis], ['LWK', 'GWD', 'YRI', 'MSL'])
...
def draw_tree_ax(paramDict, ax2, recent = False, veryrecent = False, printvals = True):
	...
	if paramDict[('Ne_change', 1)][0] != 'exp':
		rect1 = drawRectangle(LWK_axis, 0, leaf_widths[0], LWK_Ne_times[0], colorDict['LWK'], 'LWK', limits, printvals)
		ax2.add_patch(rect1)
	if paramDict[('Ne_change', 2)][0] != 'exp':	
		rect2 = drawRectangle(GWD_axis, 0, leaf_widths[1], GWD_Ne_times[0], colorDict['GWD'], 'GWD', limits, printvals)
		ax2.add_patch(rect2)
	if paramDict[('Ne_change', 3)][0] != 'exp':		
		rect3 = drawRectangle(MSL_axis, 0, leaf_widths[2], MSL_Ne_times[0], colorDict['MSL'], 'MSL', limits, printvals)
		ax2.add_patch(rect3)
	if paramDict[('Ne_change', 4)][0] != 'exp':	
		rect4 = drawRectangle(YRI_axis, 0, leaf_widths[3], YRI_Ne_times[0], colorDict['YRI'], 'YRI', limits, printvals)
		ax2.add_patch(rect4)
	...
	admix = drawAdmixture(GWD_axis, YRI_axis, admix_rate, admix_time, colorDict['YRI'], printvals) #hard-coded to assume GWD->ASI/YRI
	ax2.add_patch(admix)
	...
	coalpops = ['LWK', 'GWD', 'MSL', 'YRI']
	travellendict = {'GWD':75, 'MSL':75, 'YRI':25}
	for i in range(1, len(coalpops)):
		coaltime = paramDict[('split', i + 1)][0]
		divpop = coalpops[i]
		parentpop = coalpops[i-1]
	...
	ax2.set_ylabel('time bp (gens)')
	ax2.set_xticks([LWK_axis, GWD_axis, YRI_axis, MSL_axis])
	ax2.set_xticklabels(['LWK', 'GWD', 'YRI', 'MSL'])
