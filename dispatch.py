...
baseDirs = {'sims':'/idi/sabeti-scratch/jvitti/clean/sims/', 'scores':'/idi/sabeti-scratch/jvitti/clean/scores/'}
...
def main():
	...
	if runPoppair:
		...
		for model in models:
			...
			for modelpop in modelpops:
				...
				altpops.remove(modelpop)
				for altpop in altpops:
					argstring = "--model " + model + " --simpop " + str(modelpop) + " --altpop " + str(altpop) + " --writedir " + basedir + " --nrep " + str(numNeutReps)
					args.append(argstring)
	if compositeSims:
	...
	if repViz:
		...
		for model in models:
			for modelpop in modelpops:
				argstring = "--model " + model + " --simpop " + str(modelpop)
				args.append(argstring)
	if distViz:
		cmd = cmdPrefix + "distviz --sim"
		for model in models:
	...
	if runFPR:
		savebasedir = "/idi/sabeti-scratch/jvitti/cms2_power/fpr_4c/"
		cmd = cmdPrefix +'fpr'
		for model in models:
